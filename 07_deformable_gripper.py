# soft body:
  # simple test with less complexity
    # sphere on top of pad on top of the white cube or push a sphere onto the soft body 
    # stay in one place. if it crinkles up, it is probably a time step issue
    # pad on one side
  # double check edge points
  # collision 
    # rest pad on top of gripper horizontally
  # timestep 
  # meshlab might be helpful to find the vertex indices
...
def soft_body(boxId):
    ...
    p.createSoftBodyAnchor(cube2Id ,4,boxId,3)
    p.createSoftBodyAnchor(cube2Id ,5,boxId,3)
    p.createSoftBodyAnchor(cube2Id ,6,boxId,3)
    p.createSoftBodyAnchor(cube2Id ,7,boxId,3)

    # i = 8
    # while i <= 27:
    #   p.createSoftBodyAnchor(cubeId ,i,boxId,5)
    #   p.createSoftBodyAnchor(cube2Id ,i+20,boxId,3)
    #   i += 1
